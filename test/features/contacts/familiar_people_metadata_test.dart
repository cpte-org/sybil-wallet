...
void main() {
  ...
  test('notes and pins use strict encrypted account/network storage', () async {
    ...
    final repository = FamiliarPeopleMetadataRepository(store: store);
    final identity = testIdentity(1);
    await repository.save(testContactScope, {
      identity: const FamiliarPersonMetadata(
    ...
    final loaded = await FamiliarPeopleMetadataRepository(
      store: store,
  ...
  test(
    ...
    () async {
      ...
      final repository = FamiliarPeopleMetadataRepository(store: store);
      await repository.save(testContactScope, {
        testIdentity(1): const FamiliarPersonMetadata(notes: 'Private'),
      }, beforeWrite: () {});
      ...
      await expectLater(
        repository.save(testContactScope, {
          testIdentity(1): const FamiliarPersonMetadata(notes: 'Bad\u0000note'),
        }, beforeWrite: () {}),
        throwsA(isA<ContactFailure>()),
      );
    },
  ...
  test(
    ...
    () async {
      ...
      final container = ProviderContainer(
        overrides: [
          ...
          familiarPeopleMetadataRepositoryProvider.overrideWithValue(
            FamiliarPeopleMetadataRepository(store: store),
          ),
        ],
      );
      ...
      await container.read(familiarPeopleMetadataProvider.future);
      store.writeGate = Completer<void>();
      final operation = container
          .read(familiarPeopleMetadataProvider.notifier)
          .save(
            person,
            const FamiliarPersonMetadata(notes: 'Local only', pinned: true),
          );
