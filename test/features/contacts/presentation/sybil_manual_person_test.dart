...
import 'package:zcash_wallet/src/features/contacts/presentation/sybil_add_person_screen.dart';
...
Future<void> capture(WidgetTester t, String name) async {
  if (!const bool.fromEnvironment('SYBIL_CAPTURE')) return;
  final boundary = t.renderObject<RenderRepaintBoundary>(
    find.byKey(const Key('sybil-capture')),
  );
...
void main() {
  ...
  testWidgets(
    ...
    (t) async {
      ...
      await capture(t, 'sybil-unified-people');
  ...
  Future<ProviderContainer> show(
    ...
    final container = ProviderContainer(
      overrides: [
        ...
        sybilAddressValidatorProvider.overrideWithValue(validate),
      ],
    );
    ...
    final router = GoRouter(
      ...
      routes: [
        ...
        GoRoute(
          path: '/people/add',
          builder: (_, _) => const SybilAddPersonScreen(),
        ),
        GoRoute(
          path: '/send',
          builder: (_, state) {
            ...
            return const SybilChooseRecipientScreen();
          },
        ),
      ],
    );
