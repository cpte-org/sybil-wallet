...
class FamiliarPersonMetadata {
  const FamiliarPersonMetadata({this.notes = '', this.pinned = false});

  final String notes;
  final bool pinned;

  FamiliarPersonMetadata copyWith({String? notes, bool? pinned}) =>
  ...
  static FamiliarPersonMetadata decode(Object? value) {
    ...
    final metadata = FamiliarPersonMetadata(
...
class FamiliarPeopleMetadataRepository {
  FamiliarPeopleMetadataRepository({AppSecureStore? store})
    ...
    try {
      ...
      return Map.unmodifiable({
        for (final entry in people.entries)
          contactIdentity(entry.key): FamiliarPersonMetadata.decode(
            entry.value,
          ),
      });
    } catch (_) {
  ...
  Future<void> save(
    ContactScope scope,
    Map<String, FamiliarPersonMetadata> people, {
