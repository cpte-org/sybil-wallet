...
import '../application/sybil_people_metadata_provider.dart';
...
import 'sybil_saved_person_detail.dart';
...
class SybilPeopleScreen extends ConsumerWidget {
  const SybilPeopleScreen({super.key, this.contactId});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final metadata = ref.watch(sybilPeopleMetadataProvider);
    ...
    final content = SybilPeopleView(
      ...
      metadataError: metadata.hasError
          ...
          : null,
      onReload: () async {
        ref.invalidate(sybilPeopleMetadataProvider);
        ref.invalidate(addressBookProvider);
        await controller.reload();
      },
      onAdd: () => context.push('/people/add'),
      onIntroductions: () => open('/contacts/introductions'),
      onDelivery: () => open('/contacts/delivery'),
      onOtherNetworks: () => context.push(
        kAppFormFactor == AppFormFactor.mobile
            ? '/settings/address-book'
            : '/address-book',
      ),
      showAdvancedTools: advanced,
      onRename: rename,
      onSaveMetadata: (person, details) =>
          ref.read(sybilPeopleMetadataProvider.notifier).save(person, details),
...
class SybilPeopleView extends StatefulWidget {
  const SybilPeopleView({
    super.key,
    required this.state,
    this.contactId,
    this.metadata = const {},
  ...
  final Map<String, SybilPersonMetadata> metadata;
  final bool metadataReady;
  final Future<void> Function()? onReload;
  final VoidCallback? onAdd;
  final VoidCallback? onIntroductions, onDelivery, onOtherNetworks;
  ...
  final Future<void> Function(VerifiedContact, SybilPersonMetadata)?
  onSaveMetadata;
  final Future<void> Function(VerifiedContact)? onCheckAddress, onSuspend;

  @override
  State<SybilPeopleView> createState() => _SybilPeopleViewState();
}

class _SybilPeopleViewState extends State<SybilPeopleView> {
  ...
  @override
  void didUpdateWidget(SybilPeopleView oldWidget) {
  ...
  SybilPersonMetadata _details(VerifiedContact person) =>
      widget.metadata[person.identity] ?? const SybilPersonMetadata();
  ...
  Widget build(BuildContext context) {
    ...
    final people = [
      if (widget.state.available)
        ...widget.state.contacts.map(SybilPerson.connected),
      ...widget.savedContacts.map(SybilPerson.saved),
    ];
    final selected = people
        .where((person) => person.id == _selected)
        .firstOrNull;
    final query = _search.text.trim().toLowerCase();
    bool pinned(SybilPerson p) =>
        p.saved?.pinned ?? _details(p.connected!).pinned;
    String note(SybilPerson p) => p.saved?.note ?? _details(p.connected!).notes;
    final filtered =
        people
            .where(
              (person) =>
                        ...
                        child: Column(
                          children: [
                            _PeopleOption(
                              ...
                              title: 'Introductions',
                              subtitle:
                                  'Meet someone new through a person you both know.',
                              onTap: !widget.isLocked
                                  ? widget.onIntroductions
                                  : null,
                            ),
                            if (widget.showAdvancedTools) ...[
                              ...
                              _PeopleOption(
                                ...
                                subtitle:
                                    'Set up or inspect the SimpleX delivery connection.',
                                onTap: !widget.isLocked
                                    ? widget.onDelivery
                                    : null,
                              ),
                              _PeopleOption(
                                ...
                                subtitle:
                                    'Addresses saved for other blockchains.',
                                onTap: !widget.isLocked
                                    ? widget.onOtherNetworks
                                    : null,
                              ),
                            ],
                          ],
...
class _PersonRow extends StatelessWidget {
  ...
  final SybilPerson person;
  final SybilPersonMetadata metadata;
  final bool narrow;
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              SybilAvatar(label: person.label, identity: person.avatarIdentity),
        ...
        if (person.canPay) ...[
          SybilCard(
            ...
            child: ExpansionTile(
              ...
              subtitle: const Text('Scan or share the full connection key'),
              childrenPadding: const EdgeInsets.fromLTRB(12, 0, 12, 18),
              children: [
                ContactIdentityCode(
                  key: ValueKey(person.identity),
                  identity: person.identity,
...
class _IdentityValue extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: TextStyle(color: SybilPalette.of(context).muted)),
...
class _PeopleOption extends StatelessWidget {
  const _PeopleOption({
    required this.icon,
    required this.title,
    required this.subtitle,
