...
class FamiliarFlowPage extends StatelessWidget {
  const FamiliarFlowPage({super.key, required this.title, required this.child});
  final String title;
  final Widget child;
  @override
...
class FamiliarAddPersonScreen extends ConsumerStatefulWidget {
  const FamiliarAddPersonScreen({super.key, this.savedId});
  final String? savedId;
  @override
  ConsumerState<FamiliarAddPersonScreen> createState() =>
      _FamiliarAddPersonScreenState();
}

class _FamiliarAddPersonScreenState
    extends ConsumerState<FamiliarAddPersonScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      final valid = await ref.read(familiarAddressValidatorProvider)(address);
      if (!current()) return;
      if (!valid) {
