...
import '../../contacts/domain/sybil_person.dart';
...
import '../../contacts/application/sybil_people_metadata_provider.dart';
import '../../contacts/domain/contact_models.dart';
import '../../send/models/send_prefill_args.dart';

/// The real wallet's Sybil home. Balances, recipients and transaction rows
/// come from the existing wallet services; this widget grants no new authority.
class SybilHomeDashboard extends StatelessWidget {
  const SybilHomeDashboard({
    super.key,
    required this.sync,
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            key: const ValueKey('sybil_available_balance'),
          ...
          Wrap(
            ...
            children: [
              AppButton(
                key: const ValueKey('sybil_home_send'),
                onPressed: onSend,
                leading: const Icon(Icons.north_east, size: 18),
                child: const Text('Send'),
              ),
              AppButton(
                key: const ValueKey('sybil_home_receive'),
                onPressed: onReceive,
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (!mobile) ...[
                    const SybilPageHeader(title: 'Wallet'),
                    ...
                    builder: (context, constraints) {
                      ...
                      return Row(
                        ...
                        children: [
                          ...
                          const Expanded(child: SybilPeoplePocket()),
                        ],
                      );
                    },
                  ),
...
class SybilBalanceSyncStatus extends StatelessWidget {
  const SybilBalanceSyncStatus({
    super.key,
    required this.sync,
    this.networkPrivacy = const NetworkPrivacyState.off(),
  });
    ...
    return Tooltip(
      ...
      child: Semantics(
        ...
        child: ExcludeSemantics(
          child: Row(
            key: const ValueKey('sybil_balance_sync'),
            children: [
              Icon(
                failed
                    ? Icons.error_outline
...
class SybilPeoplePocket extends ConsumerWidget {
  const SybilPeoplePocket({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final metadata = ref.watch(sybilPeopleMetadataProvider).asData?.value;
    final contacts = available && data.available && !data.loading
        ? data.contacts
        : const <VerifiedContact>[];
    final saved = ref.watch(addressBookProvider);
    final payable = [
      ...contacts.where((c) => c.canPay).map(SybilPerson.connected),
