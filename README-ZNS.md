...
Sybil includes a Names screen for registering, resolving and managing Zcash names through a Base registry. The integration supports software accounts and has desktop and mobile layouts; hardware-account signing is not implemented.

**Current policy checkpoint: 20 September 2026.** Mainnet beta builds use Sybil's deployed API and the verified Base mainnet registry at `0x17ea278Fe9bEe80449e7E576Fb8fa4eC2f0EC3A5`. A ZEC-funded mainnet name registration completed through the wallet. This is live integration evidence for that run; it is not an independent security audit or complete qualification of release, funds-return, mobile background delivery, or every physical device. See the [contract deployment record](../ZNS-contract/docs/MAINNET-DEPLOYMENT.md), the [Names feature notes](lib/src/features/zns/README.md), and the [beta release notes](release_notes/v1.0.0-beta.1.md) for scope and limits.
...
Select **Verify & save settings**. Deployment settings are saved locally. The [mainnet beta build](docs/SYBIL-BETA-BUILD.md) sets `ZCASH_DEFAULT_NETWORK=main`, uses the verified Base mainnet registry and batch-account defaults, and disables the Base Sepolia preset. Existing saved settings take precedence. The older testnet development script remains available separately. The client verifies chain, protocol ID, registry/token compatibility and canonical snapshot blocks. Test accounts need test ETH/tokens supplied separately; live funding requires Zcash and Base mainnet.
